    ...
    <div class="content_box">
      <div class="content orders_container">
        <div class="switch_orders">
          <router-link class="unpaid tab" to="/zxbOrders?type=unpaid" @click.native="toggle('unpaid')" tag="span">
            <span :class="{'tab_on': $route.query.type === 'unpaid'}">待付款</span>
          </router-link>
          <router-link class="paid tab"  to="/zxbOrders?type=paid" @click.native="toggle('paid')" tag="span">
        ...
        <div class="main">
          ...
          <div class="unpaidList" v-show="$route.query.type == 'unpaid'">
            <div v-for="(order, index) in unpaidOrders" class="cell">
              ...
              <p>车牌号码: &nbsp;&nbsp;&nbsp;&nbsp;头：{{ order.plate_no }} &nbsp;&nbsp;&nbsp;&nbsp; 挂：{{ order.plate_extra_no }}</p>
...
<script>
  ...
  import { zxbOrderList,zxbOrderCancel,zxbOrderSave} from '../../assets/js/api'
  ...
  export default {
    ...
    data(){
      return {
        getPaidOrdersFunc : zxbOrderList, // 获取已完成订单的api
        unpaidStatus : 1, // 未完成订单的状态码，查询多个可把对应状态码累加
        paidStatus : 4, // 已完成订单的状态码
        cancelOrderFunc : zxbOrderCancel, // 取消本产品订单的api
    ...
    methods: {
      ...
      showDetail(order){
        let info = {orderId:order.id,productType:"zxb"};
...
//        window.location.href="./vueStatic/paySuccess.html?orderId="+order.id+"&productType=zxb";
      },
      toPay(order){
//          this.$router.push({ name: 'zcbCreate', query: { status:(order.is_trial ? 0 : 1)+'_' +  order.id }})
        this.$router.push({ name: 'zxbConfirm', query: {orderId:order.id }})
      },
      // 取消订单
