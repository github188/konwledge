  ...
  <div class="spb_orders">
    ...
    <div class="content_box">
      <div class="content orders_container">
        <div class="tab">
          <router-link class="unpaid tab" to="/hcbOrders?type=unpaid" @click.native="toggle('unpaid')" tag="div">
            <span :class="{'tab_on': $route.query.type === 'unpaid'}">未生效保单</span>
          </router-link>
          <router-link class="paid tab"  to="/hcbOrders?type=paid" @click.native="toggle('paid')" tag="div">
...
<script>
  ...
  export default {
    ...
    methods:{
      detail(order){
        return '/hcbOrderDetails?orderId='+ order.id + '&orderStatus=' + order.status ;
      },
    }
  }
</script>
