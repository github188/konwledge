      ...
      <div class="content">
        <ul class="order_list">
          ...
          <div class="car-panel" v-for="(item,index) in truckList" :key="index">
            ...
            <Panel :flag="item.isShow" :change="item">
              <li class="li-tab">
              ...
              <li class="dchynb-tab">
                <div class="li-tab">
                  ...
                  <div class="li-tab-body f24">
                    <div v-for="(price,PriceIndex) in  priceData" :key="PriceIndex+'a'" class="tab-item" :class="{active:item.limitType==PriceIndex}" @click="item.limitType=PriceIndex">{{price.coverage/10000}}万</div>
                  </div>
                </div>
                <div class="tab-tip">提示：10吨以上车辆最低起保限额50万元起，否则无效。</div>
              </li>
              ...
              <li class="li-tab driving">
                <span class="label driving-license">行驶证所有人名称</span>
                <div class="li-tab-body">
                  <div class="tab-item" :class="{active:item.driving_license_with_beneficiary}" @click="item.driving_license_with_beneficiary=true">同被保险人</div>
                  <div class="tab-item" :class="{active:!item.driving_license_with_beneficiary}" @click="item.driving_license_with_beneficiary=false; item.lease_contract_or_attachment_instructions=[]">两者不同</div>
                </div>
              </li>
              <li class="article" v-show="!item.driving_license_with_beneficiary">
                <span>上传车辆租凭合同或挂靠说明</span>
                <div class="upload">
                  <upload :url="$store.state.fileUrl" v-model="item.lease_contract_or_attachment_instructions" :maxLimit="15"></upload>
                </div>
              </li>
              <li class="article">
                <span>上传车辆运输证</span>
