    ...
    <div class="content_box">
      ...
      <div class="confirm_bottom">
        ...
        <div class="foot" v-if="orderStatus == 2">
          <router-link :to="{path:'/dchynbCreate',query:{orderId:orderId}}" class="btn" >返回并修改</router-link>
          <a @click="open" class="btn bg_blue" >确认并支付</a>
        </div>
        <div class="foot" v-if="orderStatus == 1">
          <router-link :to="{path:'/dchynbCreate',query:{orderId:orderId}}" class="btn" >返回并修改</router-link>
          <router-link :to="{path:'/policyDown',query:{orderId:orderId,product:'dchynb'}}" class="btn bg_blue" >下载投保单</router-link>
        </div>
        <router-link tag="div" class="foot one" :to="{path:'/dchynbCreate'}" v-if="orderStatus == 8 || orderStatus == 16">
          继续投保
        </router-link>
      </div>
    </div>
    <Pay ref="pay" v-if="orderStatus == 2" :productType="'hcb'" :orderId="$route.query.orderId" :backOrderLink="'/dchynbOrders?type=unpaid'" :cancelLink="'/dchynbOrders?type=unpaid'">
    </Pay>
  </div>
</template>
<script>
