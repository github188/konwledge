export default class Editor {
  constructor({
    ...
    this.originRegion = {width: 0, height: 0}
  ...
  * */
   ...
   */
  isScale () {
    return this.originRegion.width*1 != 0 && this.originRegion.height*1 != 0
  }
   ...
   * @param originRegion  原始区域的宽高对象 eg: {width: 1920, height: 1080}
   * @returns {*} canvas显示区域数组
   */
  getCanvasRegion(originArr, originRegion){
    let scaleRatio = this.getScaleRatio(originRegion)
  ...
  //以下为提供给外部调用的方法
  /**
   * 获取画框的数据: （自动判断是否需要缩放） 若需要缩放，则返回缩放后的区域数组；否则返回实际区域数组
   * ps： 判断是否需要缩放取决于设置区域的时候是否有挂originRegion
   * @returns {Array|*}
   */
  getRegion() {
    return this.isScale() ? this.getOriginRegion(this.boxArr, this.originRegion) : this.boxArr
  }
  ...
  /**
   * 设置画框区域：传originRegion时，自动将（视频/图片上的）原始区域数组缩放为canvas区域数组再设置canvas区域；否则直接设置canvas区域
   * @param originArr （视频/图片上的）原始区域数组
   * @param originRegion  原始区域的宽高对象 eg: {width: 1920, height: 1080}
   */
  setRegion(originArr, originRegion) { //设置画框区域和数据
    originRegion && (this.originRegion = originRegion);
    this.boxArr = originRegion ? this.getCanvasRegion(originArr, originRegion) : originArr
