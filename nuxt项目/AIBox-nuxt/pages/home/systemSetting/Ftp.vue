<template>
  <fill-content ref="fillContent" :title="$t('menu.net.ftp')" class-name="ftp-page">
    <div class="auto-checked-box checked-box">
      <p class="checked-wrapper">
        <el-checkbox v-model="ftpInfo.ftpenable">{{ $t('ftp.ftpUse') }}</el-checkbox>
      </p>
      <p class="server-box">
        <label class="label f14">{{ $t('ftp.pip') }}：
          <el-input v-model.trim="ftpInfo.ftpip"></el-input>
        </label>
      </p>
      <p class="time-box f14">
        <label class="label">{{ $t('ftp.port') }}：
          <el-input v-model.trim="ftpInfo.ftpport"></el-input>
        </label>
      </p>
      <p class="time-box f14">
        <label class="label">{{ $t('ftp.username') }}：
          <el-input v-model.trim="ftpInfo.ftpuser"></el-input>
        </label>
      </p>
      <p class="time-box f14">
        <label class="label">{{ $t('ftp.password') }}：
          <el-input v-model.trim="ftpInfo.ftppassword"></el-input>
        </label>
      </p>
      <p class="time-box f14">
        <label class="label">{{ $t('ftp.url') }}：
    ...
    <div class="hand-checked-box checked-box">
      <p class="checked-wrapper f14">
        <el-checkbox v-model="ftpInfo.httpenable">{{ $t('ftp.httpUse') }}</el-checkbox>
      </p>
      <p class="time-box f14 http-choose">
        <label class="label">{{ $t('ftp.httpUse') }}：
          <el-input v-model.trim="ftpInfo.httpurl"></el-input>
        </label>
      </p>
    </div>
    <p class="save-box">
      <el-button type="primary" @click="setFtpData">{{ $t('ftp.saveBtn') }}</el-button>
    </p>
  </fill-content>
</template>
